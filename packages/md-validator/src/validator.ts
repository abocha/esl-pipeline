...
const FrontSchema = z.object({
  ...
  input_type: z.enum(['generate', 'authentic']),
...
function extractFirstCodeBlock(raw: string): { lang: string; content: string } {
  ...
  if (!m) throw new Error('No fenced code block found. Output must be inside a single triple-backtick block.')
...
function sectionSlice(root: Root, startHeading: Heading): RootContent[] {
  ...
  const rel = ch.slice(idx + 1).findIndex(
    (n: any) => n.type === 'heading' && n.depth <= (startHeading as any).depth
  )
...
export async function validateMarkdownFile(rawFile: string, opts: ValidateOptions = {}): Promise<ValidateResult> {
  ...
  // b) markers
  const hasAnswerKeyToggle = fm.content.includes(':::toggle-heading Answer Key')
  const hasTeacherPlanToggle = fm.content.includes(":::toggle-heading Teacher’s Follow-up Plan") ||
                               fm.content.includes(":::toggle-heading Teacher's Follow-up Plan")
  if (!hasAnswerKeyToggle) errors.push('Missing marker: ":::toggle-heading Answer Key"')
  if (!hasTeacherPlanToggle) errors.push('Missing marker: ":::toggle-heading Teacher’s Follow-up Plan"')
  ...
  if (!study) {
    ...
    if (meta?.speaker_labels && meta.speaker_labels.length > 0) {
      const labelSet = new Set(meta.speaker_labels)
      const lines = study.body.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
      for (const [i, line] of lines.entries()) {
        ...
        if (!labelSet.has(who)) {
          errors.push(`study-text line ${i + 1}: unknown speaker "${who}". Allowed: [${[...labelSet].join(', ')}].`)
        }
      }
    } else {
  ...
  return { ok: errors.length === 0, errors, warnings, meta: meta as any }
}
