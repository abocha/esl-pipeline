...
const CLEANUP_INTERVAL = 60000; // Clean up stale locks every 60 seconds

export class FfmpegSemaphore {
    private static instances: Set<FfmpegSemaphore> = new Set();
    private redis: Redis;
    ...
    private startCleanup(): void {
        this.cleanupTimer = setInterval(() => {
            this.cleanupStaleLocks().catch(err => {
                logger.error(err, { message: 'FFmpeg semaphore cleanup failed' });
            });
        }, CLEANUP_INTERVAL);
    }
...
export function getFfmpegSemaphore(maxConcurrent?: number): FfmpegSemaphore {
    if (!semaphoreInstance) {
        if (maxConcurrent === undefined) {
            throw new Error('FFmpeg semaphore not initialized. Call getFfmpegSemaphore(maxConcurrent) first.');
        }
