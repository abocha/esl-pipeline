...
describe('addOrReplaceAudioUnderStudyText', () => {
  it('throws on missing pageId', async () => {
    await expect(addOrReplaceAudioUnderStudyText('', 'https://example.com/audio.mp3')).rejects.toThrow('pageId is required');
  });
  ...
  it('appends audio when no existing audio and replace=false', async () => {
    ...
    mockClient.blocks.children.list.mockResolvedValueOnce({
      results: [{ type: 'toggle', id: 'toggle123', toggle: { rich_text: [{ plain_text: 'study-text' }] } }],
      next_cursor: null
    ...
    const result = await addOrReplaceAudioUnderStudyText('page123', 'https://example.com/audio.mp3', opts);
